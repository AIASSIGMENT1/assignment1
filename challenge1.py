...
# --------------------------------------------------------------------
# BUILD A YEARLY PANEL: sales, leads, conversion, satisfaction 
# --------------------------------------------------------------------
yearly_funnel = (
    transactions.groupby(['year', 'status'])['customer_id']
    .nunique()
...
# Ensure expected columns exist even if 0 in some years
for col in ['filled in form','sales','paid deposit','cancelled','not reachable']:
    if col not in yearly_funnel.columns:
        yearly_funnel[col] = 0
...
print("\nYEARLY PANEL (head)")
print(yearly[['year','leads','sales','conversion_rate','organization','global_satisfaction']])

# ---- QUANTIFY RELATIONSHIPS (Pearson) ----
corr_to_sales = yearly[['sales','leads','conversion_rate','organization','global_satisfaction']].corr()['sales'].sort_values(ascending=False)
print("\nPearson r with SALES")
print(corr_to_sales)

# ---- SCATTERPLOTS WITH TREND LINES (no seaborn) ----
import numpy as np
import matplotlib.pyplot as plt
